...
export function getGroupedComponents() {
  return getDocByType().reduce((acc, doc) => {
...
const getUsageDoc = (id: string) => {
  return allCompos.find((_doc) => _doc.id === id && _doc.scope === 'usage')
}

const getThemingDoc = (id: string) => {
  return allCompos.find((_doc) => _doc.id === id && _doc.scope === 'theming')
}

const getPropsDoc = (id: string) => {
  return allCompos.find((_doc) => _doc.id === id && _doc.scope === 'props')
}
