...
const Node: FC<{ node: TreeNode; level: number; activePath: string }> = ({ node, level, activePath }) => {
  ...
  return (
    <>
      ...
      />
      {node.children.length > 0 && !collapsed && <Tree tree={node.children} level={level + 1} activePath={activePath} />}
    </>
  )
}

const Tree: FC<{ tree: TreeNode[]; level: number; activePath: string }> = ({ tree, level, activePath }) => (
  <div className={classNames('ml-3 space-y-2 pl-3', level > 0 ? 'border-l border-gray-200 dark:border-gray-800' : '')}>
